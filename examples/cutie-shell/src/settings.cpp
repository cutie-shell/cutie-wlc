...
Settings::Settings(QObject *parent) : QObject(parent) {
    ...
    ((QQmlEngine *)parent)->rootContext()->setContextProperty("screenBrightness", this->settingsStore->value("screenBrightness", 100));
}
...
void Settings::refreshBatteryInfo() {
    ...
    ((QQmlEngine *)parent())->rootContext()->setContextProperty("batteryStatus", upower_display);
}
...
void Settings::execApp(QString command)
{
    ...
    qputenv("XDG_RUNTIME_DIR", QByteArray("/run/user/32011"));
    qputenv("_JAVA_AWT_WM_NONREPARENTING", QByteArray("1"));
        ...
        if (curAppDir->exists()) {
			...
			appName = appName.mid(0,14).append("...");
                    ...
                    if (appName != "" && appExec != "" && appHidden != "true" && appNoDisplay != "true")
                        emit addApp(appName, appExec, appIcon);
                        //QMetaObject::invokeMethod(((QQmlEngine *)parent())->rootContext(), "addApp", Q_ARG(QVariant, appName), Q_ARG(QVariant, appExec), Q_ARG(QVariant, appIcon));
                }
                delete curEntryFile;
            }
        }
...
void Settings::onAtmospherePathChanged() {
    ...
    ((QQmlEngine *)parent())->rootContext()->setContextProperty("atmospherePath", apath);
...
void Settings::onAtmosphereVariantChanged() {
    ...
    ((QQmlEngine *)parent())->rootContext()->setContextProperty("atmosphereVariant", avar);
