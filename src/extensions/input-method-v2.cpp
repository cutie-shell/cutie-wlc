...
void InputMethodManagerV2::zwp_input_method_manager_v2_destroy_resource(
	Resource *resource)
{
	...
	Resource *resource, struct ::wl_resource *seat, uint32_t input_method)
{
	if (m_inputmethod == nullptr) {
		...
		connect(m_textinputV2, &TextInputManagerV2::showInputPanel,
			m_inputmethod, &InputMethodV2::onShowInputPanel);
		connect(m_textinputV2, &TextInputManagerV2::hideInputPanel,
			m_inputmethod, &InputMethodV2::onHideInputPanel);
		connect(m_textinputV2, &TextInputManagerV2::contentTypeChanged,
			m_inputmethod, &InputMethodV2::onContentTypeChanged);

		connect(m_textinputV3, &TextInputManagerV3::showInputPanel,
			m_inputmethod, &InputMethodV2::onShowInputPanel);
		...
		qWarning()
			<< "Another input method active, ignoring this request!!!";
	}
}

void InputMethodManagerV2::zwp_input_method_manager_v2_destroy(
	Resource *resource)
{
}
...
InputMethodV2::InputMethodV2(struct ::wl_client *client, uint32_t id,
			     int version, CwlCompositor *compositor)
...
void InputMethodV2::zwp_input_method_v2_delete_surrounding_text(
	Resource *resource, uint32_t before_length, uint32_t after_length)
{
}

void InputMethodV2::zwp_input_method_v2_commit(Resource *resource,
					       uint32_t serial)
{
...
void InputMethodV2::zwp_input_method_v2_get_input_popup_surface(
	Resource *resource, uint32_t id, struct ::wl_resource *surface)
{
}

void InputMethodV2::zwp_input_method_v2_grab_keyboard(Resource *resource,
						      uint32_t keyboard)
{
...
void InputMethodV2::sendWithModifier()
{
	...
	QKeyEvent *key_press =
		new QKeyEvent(QEvent::KeyPress, qtkey[0].key(), modifiers);
	QKeyEvent *key_release = new QKeyEvent(
		QEvent::KeyRelease, qtkey[0].key(), Qt::KeyboardModifiers{});
