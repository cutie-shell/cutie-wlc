...
void ScreencopyManagerV1::zwlr_screencopy_manager_v1_capture_output(
	Resource *resource, uint32_t id, int32_t overlay_cursor,
	struct ::wl_resource *output)
{
	GlWindow *window = m_compositor->glWindow();
	zwlr_screencopy_manager_v1_capture_output_region(resource, id,
							 overlay_cursor, output,
							 0, 0, window->width(),
							 window->height());
}
...
void ScreencopyManagerV1::zwlr_screencopy_manager_v1_capture_output_region(
	Resource *resource, uint32_t id, int32_t overlay_cursor,
	struct ::wl_resource *output, int32_t x, int32_t y, int32_t width,
	int32_t height)
{
	ScreencopyFrameV1 *frame = new ScreencopyFrameV1(resource->client(), id,
							 resource->version());
	QImage fb = m_compositor->glWindow()->grabFramebuffer().copy(
		x, y, width, height);
	fb.convertTo(QImage::Format_ARGB32_Premultiplied);
	frame->setFrameBuffer(fb, QDateTime::currentSecsSinceEpoch(),
			      (QDateTime::currentMSecsSinceEpoch() % 1000) *
				      1000000);
	frame->send_buffer(0 /* 0 = ARGB32 */, fb.width(), fb.height(),
			   fb.bytesPerLine());
	frame->send_buffer_done();
}

ScreencopyFrameV1::ScreencopyFrameV1(wl_client *client, uint32_t id,
				     int version)
	: QtWaylandServer::zwlr_screencopy_frame_v1(client, id, version)
{
}

void ScreencopyFrameV1::setFrameBuffer(QImage image, uint64_t tv_sec,
				       uint32_t tv_nsec)
{
	m_frameBuffer = image;
	m_tv_sec = tv_sec;
...
void ScreencopyFrameV1::zwlr_screencopy_frame_v1_copy(
	Resource *resource, struct ::wl_resource *buffer)
{
	...
	if (width != m_frameBuffer.width() ||
	    height != m_frameBuffer.height() ||
	    bytesPerLine != m_frameBuffer.bytesPerLine()) {
		send_failed();
		return;
	}
